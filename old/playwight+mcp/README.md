...
Ein Docker-Compose-Setup mit einem einzigen Container. Das Image basiert auf dem offiziellen `mcr.microsoft.com/playwright`-Image und erweitert es um:

- `@playwright/mcp` – Playwright MCP-Server
- `desktop-lite` – TigerVNC + noVNC + Fluxbox aus den Dev Containers Features

Alle Dienste laufen in einem Container, deshalb entfällt die CDP-Verbindung zwischen getrennten Containern.
...
Wenn `MCP_AUTH_TOKEN` leer bleibt, erzeugt der Container beim Start automatisch einen zufälligen Token und zeigt ihn in den Logs an.
...
Chromium läuft als persistenter Prozess mit aktiviertem CDP. Der MCP-Server verbindet sich intern über CDP damit, sodass das Browserfenster auch bei Wechsel der MCP-Verbindung erhalten bleibt.
...
Danach siehst du den Fluxbox-Desktop. Chromium wird vom MCP-Server bei Bedarf gestartet.

## Wichtige Hinweise

- Der Container läuft als `root` und startet Chromium mit `--no-sandbox`. Das ist für eine lokale Entwicklungs-/Debug-Umgebung gedacht, nicht für den Zugriff auf nicht vertrauenswürdige Seiten.
- `shm_size: 2gb` und `init: true` entsprechen den Empfehlungen aus der [Playwright-Docker-Dokumentation](https://playwright.dev/docs/docker).
- Die Browser-Version im Playwright-Image muss mit der Version kompatibel sein, die `@playwright/mcp` intern verwendet. Falls es zu Fehlern kommt, das Playwright-Base-Image (`v1.62.0-noble`) oder die MCP-Version in `.env` anpassen.
...
- `PLAYWRIGHT_MCP_VERSION` – MCP-Version
- `PLAYWRIGHT_MCP_PORT` – Port des MCP-Servers
- `CDP_PORT` – Port des Chrome DevTools Protocol-Endpunkts
